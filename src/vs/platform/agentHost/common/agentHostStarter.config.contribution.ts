...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[AgentHostByokModelsEnabledSettingId]: {
			...
			// The Agents window owns a targeted model pool. Keep the bridge enabled by
			// default so the same user-configured models that are available in Chat are
			// copied into that pool instead of leaving Copilot CLI with only "Auto".
			default: true,
			tags: ['experimental', 'advanced'],
			experiment: { mode: 'startup' },
		},
