...
/** Whisper Tiny exported to ONNX with the Q8 quantized weights. */
export const DEFAULT_LOCAL_TRANSCRIPTION_MODEL = 'onnx-community/whisper-tiny';
...
export interface ILocalTranscriptionModelStatus {
	...
	/** Bytes downloaded across the model files, when Hugging Face reports them. */
	readonly downloadedBytes?: number;
	/** Total bytes across the model files, when known. */
...
export interface ILocalTranscriptionService {
	...
	/** Download/cache and prepare the model asynchronously for microphone use. */
	prepareModel(options: { readonly cacheDir: string }): Promise<void>;
