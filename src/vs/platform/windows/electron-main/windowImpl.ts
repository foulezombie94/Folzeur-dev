...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			if (isWindows) {
				try {
					const iconPath = FileAccess.asFileUri('resources/win32/code.ico').fsPath;
					this._win.setIcon(electron.nativeImage.createFromPath(iconPath));
				} catch (e) { }
			}
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(null)): void {
		...
		if (process.env.VSCODE_DEV && process.env.VSCODE_DEV_SERVER_URL) {
			if (configuration.isSessionsWindow) {
				const baseUrl = process.env.VSCODE_DEV_SERVER_URL.endsWith('/') ? process.env.VSCODE_DEV_SERVER_URL : `${process.env.VSCODE_DEV_SERVER_URL}/`;
				windowUrl = `${baseUrl}vs/sessions/electron-browser/sessions-dev.html`;
			} else {
