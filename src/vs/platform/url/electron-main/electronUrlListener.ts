...
export class ElectronURLListener extends Disposable {
	...
	constructor(
		...
		if (isWindows && !environmentMainService.isPortable) {
			// Electron's `setAsDefaultProtocolClient` expects each argument without
			// command-line quoting. It builds the Windows command line itself. Passing
			// quotes here makes them part of the app path in development builds, so
			// Electron tries to resolve a path such as `"C:\\...\\LOCALVSCODE"`.
			const windowsParameters = environmentMainService.isBuilt ? [] : [environmentMainService.appRoot];
			windowsParameters.push('--user-data-dir', environmentMainService.userDataPath, '--open-url', '--');
