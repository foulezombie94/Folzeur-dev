...
import { CompositeActionViewItem, CompositeOverflowActivityAction, CompositeOverflowActivityActionViewItem, CompositeViewsActivityAction, CompositeViewsActivityActionViewItem, CompositeBarAction, ICompositeBar, ICompositeBarColors, IActivityHoverOptions } from './compositeBarActions.js';
...
export class CompositeBar extends Widget implements ICompositeBar {
	...
	private compositeViewsAction = this._register(new MutableDisposable<CompositeViewsActivityAction>());
	private compositeViewsActionViewItem = this._register(new MutableDisposable<CompositeViewsActivityActionViewItem>());
	...
	create(parent: HTMLElement): HTMLElement {
		...
		this.compositeSwitcherBar = this._register(new ActionBar(actionBarDiv, {
			actionViewItemProvider: (action, options) => {
				...
				if (action instanceof CompositeViewsActivityAction) {
					return this.compositeViewsActionViewItem.value;
				}
	...
	private updateCompositeSwitcher(donotTrigger?: boolean): void {
		...
		// Pull trailing actions before updating visible composites
		if (this.compositeOverflowAction.value) {
			compositeSwitcherBar.pull(compositeSwitcherBar.length() - 1);
			this.compositeOverflowAction.value = undefined;
			this.compositeOverflowActionViewItem.value = undefined;
		}
		if (this.compositeViewsAction.value) {
			compositeSwitcherBar.pull(compositeSwitcherBar.length() - 1);
			this.compositeViewsAction.value = undefined;
			this.compositeViewsActionViewItem.value = undefined;
		}
		...
		// Add views action always
		if (!this.compositeViewsAction.value) {
			this.compositeViewsAction.value = this.instantiationService.createInstance(CompositeViewsActivityAction, () => {
				this.compositeViewsActionViewItem.value?.showMenu();
			});
			this.compositeViewsActionViewItem.value = this.instantiationService.createInstance(
				CompositeViewsActivityActionViewItem,
				this.compositeViewsAction.value,
				() => this.getContextMenuActions(),
				this.options.colors,
		...
		if (this.compositeViewsAction.value) {
			compositeSwitcherBar.push(this.compositeViewsAction.value, { label: false, icon: true });
		}
