...
class LayoutStateModel extends Disposable {
	...
	private applyOverrides(configuration: ILayoutStateLoadConfiguration): void {
		...
		// Ensure primary left sidebar is always visible on startup
		this.setRuntimeValue(LayoutStateKeys.SIDEBAR_HIDDEN, false);

		// Auxiliary bar & Primary sidebar settings for Agent Sessions
		...
		if (startupEditor === 'agentSessionsWelcomePage' || this.environmentService.isSessionsWindow) {
