		...
		const sendRequestInternal = async () => {
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					// Folzeur's native.api agent starts MCP lazily when the request router sets needsMcp — skip eager autostart so a simple "ça va" does not prompt for Gmail/Supabase gloves.
					if (model.canUseTools && agent.id !== 'native.api') {
