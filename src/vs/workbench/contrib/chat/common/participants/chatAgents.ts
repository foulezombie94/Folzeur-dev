...
export class ChatAgentService extends Disposable implements IChatAgentService {
	...
	getDefaultAgent(location: ChatAgentLocation, mode: ChatModeKind = ChatModeKind.Ask): IChatAgent | undefined {
		const defaultAgents = this.getActivatedAgents().filter(a => {
		...
		let preferred = this._preferExtensionAgent(defaultAgents);
		if (preferred && preferred.id.startsWith('setup.')) {
			preferred = undefined; // Force fallback if the only default is a setup agent
		}
		if (preferred) {
			return preferred;
		}
		const fallbackAgents = this.getActivatedAgents().filter(a => a.locations.includes(location));
		return this._preferExtensionAgent(fallbackAgents) ?? this.getActivatedAgents().at(-1);
	}
	...
	getContributedDefaultAgent(location: ChatAgentLocation): IChatAgentData | undefined {
		const defaultAgents = this.getAgents().filter(a => !!a.isDefault && a.locations.includes(location));
		let preferred = this._preferExtensionAgent(defaultAgents);
		if (preferred && preferred.id.startsWith('setup.')) {
			preferred = undefined;
		}
		...
		return this._preferExtensionAgent(this.getAgents().filter(a => a.locations.includes(location))) ?? this.getAgents().at(-1);
	}
	...
	private _preferExtensionAgent<T extends IChatAgentData>(agents: T[]): T | undefined {
		...
		// BLOCK github.copilot-chat from hijacking the chat
		const allowedAgents = agents.filter(a => a.extensionId?.value !== 'github.copilot-chat');
		
		const nonSetupAgents = allowedAgents.filter(a => !a.id.startsWith('setup.'));
		const preferred = findLast(nonSetupAgents, agent => !agent.isCore) ?? nonSetupAgents.at(-1);
		if (preferred) {
			return preferred;
		}
		return findLast(allowedAgents, agent => !agent.isCore) ?? allowedAgents.at(-1);
	}
