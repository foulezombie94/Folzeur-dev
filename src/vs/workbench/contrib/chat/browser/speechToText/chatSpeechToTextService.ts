...
/**
 * Selects the dictation model. The on-device default is Whisper Tiny Q4
 * run through {@link ILocalTranscriptionService}; the sentinel
 * {@link DICTATION_MAI_MODEL_ID} routes to the cloud voice service instead.
 */
 ...
 * - `nemo`: an on-device model via {@link ILocalTranscriptionService} (Whisper/Transformers.js).
...
export interface IChatSpeechToTextService {
	...
	/** Downloaded model bytes, when available. */
	readonly modelDownloadedBytes?: number;
	/** Total model bytes, when available. */
...
export class ChatSpeechToTextService extends Disposable implements IChatSpeechToTextService {
	...
	private _modelDownloadedBytes: number | undefined;
	get modelDownloadedBytes(): number | undefined {
		return this._modelDownloadedBytes;
	}

	private _modelTotalBytes: number | undefined;
	get modelTotalBytes(): number | undefined {
		return this._modelTotalBytes;
	}
	...
	private _setPreparingModel(preparing: boolean): void {
		...
		if (!preparing) {
			...
			this._modelDownloadedBytes = undefined;
			this._modelTotalBytes = undefined;
	...
	private _updateModelDownloadProgress(status: ILocalTranscriptionModelStatus): void {
		this._modelDownloadedBytes = status.downloadedBytes;
		this._modelTotalBytes = status.totalBytes;
