...
export class ChatViewWelcomePart extends Disposable {
	...
	constructor(
		...
		try {
			...
			// Codex Suggested Cards Grid
			const cardsContainer = dom.append(this.element, $('.codex-action-cards-container'));
			const actionCards = [
				{ icon: Codicon.megaphone, label: 'Explorer et comprendre le code' },
				{ icon: Codicon.tools, label: 'Créer une fonctionnalité, une application ou un outil' },
				{ icon: Codicon.sync, label: 'Vérifier le code et suggérer des modifications' },
				{ icon: Codicon.bug, label: 'Corriger les problèmes et les échecs' },
			];

			for (const card of actionCards) {
				const cardEl = dom.append(cardsContainer, $('.codex-action-card'));
				const cardIcon = dom.append(cardEl, $('.codex-card-icon'));
				cardIcon.appendChild(renderIcon(card.icon));
				const cardLabel = dom.append(cardEl, $('.codex-card-label'));
