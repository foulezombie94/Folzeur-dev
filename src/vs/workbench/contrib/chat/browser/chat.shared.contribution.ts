...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'chat.api.provider': {
			type: 'string',
				enum: ['gemini', 'openai', 'claude', 'ollama', 'none'],
			enumDescriptions: [
				nls.localize('chat.api.provider.gemini', "Use Google Gemini API for native chat"),
				nls.localize('chat.api.provider.openai', "Use OpenAI API for native chat"),
					nls.localize('chat.api.provider.claude', "Use Anthropic Claude API for native chat"),
					nls.localize('chat.api.provider.ollama', "Use a local Ollama server for native chat"),
				nls.localize('chat.api.provider.none', "Disable native chat API"),
			],
			description: nls.localize('chat.api.provider', "Select which native API provider to use when answering chat requests."),
			default: 'none'
		},
		'chat.api.key.set': {
			type: 'string',
			markdownDescription: nls.localize('chat.api.key.set', "API Key for the selected native provider. [Set API Key securely](command:folzeur.agent.setApiKey). Note: The actual key is encrypted and stored in the OS Keychain."),
			default: '(Encrypted in Keychain)'
		},
		'chat.api.gemini.model': {
			type: 'string',
			enum: [
			...
			enumDescriptions: [
				nls.localize('chat.api.gemini.model.31pro', "Gemini 3.1 Pro: Modèle de pointe conçu pour le raisonnement complexe, l'analyse approfondie et les tâches difficiles."),
				nls.localize('chat.api.gemini.model.36flash', "Gemini 3.6 Flash: Dernier modèle stable équilibrant vitesse et intelligence pour des performances solides dans les tâches agentiques et multimodales."),
				nls.localize('chat.api.gemini.model.35flash', "Gemini 3.5 Flash: Modèle très intelligent offrant des performances de pointe constantes pour les tâches de codage et les agents."),
				nls.localize('chat.api.gemini.model.3flash', "Gemini 3 Flash: Version en aperçu (preview) de la gamme Flash 3."),
				nls.localize('chat.api.gemini.model.35lite', "Gemini 3.5 Flash-Lite: Sous-agent à faible latence et très économique, spécifiquement conçu pour l'automatisation à haut volume."),
				nls.localize('chat.api.gemini.model.31lite', "Gemini 3.1 Flash-Lite: Offre des performances rivalisant avec les modèles plus grands, pour un coût et une latence bien inférieurs.")
			],
			description: nls.localize('chat.api.gemini.model', "The model identifier sent to the selected provider."),
			default: 'gemini-3.6-flash'
		},
		'chat.api.claude.model': {
			type: 'string',
			description: nls.localize('chat.api.claude.model', "Model ID for Anthropic Claude (e.g. claude-3-5-sonnet-20241022)."),
			default: 'claude-3-5-sonnet-20241022'
		},
		'chat.api.openai.model': {
			type: 'string',
			description: nls.localize('chat.api.openai.model', "Model ID for OpenAI (e.g. gpt-4o-mini)."),
			default: 'gpt-4o-mini'
		},
		'chat.api.maxInputTokens': {
			type: 'integer',
			minimum: 0,
			maximum: 4_000_000,
			description: nls.localize('chat.api.maxInputTokens', "Override the selected provider model context window. Use 0 for the built-in conservative model-family default."),
			default: 0
		},
		'chat.api.maxOutputTokens': {
			type: 'integer',
			minimum: 0,
			maximum: 1_000_000,
			description: nls.localize('chat.api.maxOutputTokens', "Override the selected provider model output limit. Use 0 for the built-in conservative model-family default."),
			default: 0
		},
		'chat.api.autoApproveTools': {
			type: 'boolean',
			description: nls.localize('chat.api.autoApproveTools', "Automatically approve all tool executions (terminal commands, file edits) without asking for confirmation every time."),
			default: false
		},
		'chat.api.allowTerminal': {
			type: 'boolean',
			description: nls.localize('chat.api.allowTerminal', "Allow the native agent runtime to execute integrated terminal tools."),
			default: true
		},
		'chat.api.allowNetwork': {
			type: 'boolean',
			description: nls.localize('chat.api.allowNetwork', "Allow the native agent runtime to execute network tools."),
			default: true
		},
		'chat.api.allowWebSearch': {
			type: 'boolean',
			description: nls.localize('chat.api.allowWebSearch', "Allow the native agent to search the public web."),
			default: true
		},
		'chat.api.allowFetch': {
			type: 'boolean',
			description: nls.localize('chat.api.allowFetch', "Allow the native agent to fetch public HTTP(S) pages."),
			default: true
		},
		'chat.api.allowBrowser': {
			type: 'boolean',
			description: nls.localize('chat.api.allowBrowser', "Allow the native agent to control an isolated browser. Disabled by default because browser execution has a larger network and scripting security surface."),
			default: false
		},
		'chat.api.allowModelDownloads': {
			type: 'boolean',
		...
		'chat.api.terminalMode': {
			type: 'string',
			enum: ['ask', 'allowlist', 'auto'],
			description: nls.localize('chat.api.terminalMode', "Controls approval for terminal commands."),
			default: 'ask'
		},
		'chat.api.terminalAllowlist': {
			type: 'string',
			description: nls.localize('chat.api.terminalAllowlist', "Comma-separated command prefixes allowed without confirmation when terminal mode is allowlist."),
			default: 'git status, git diff, git log, npm test, npm run test, npm run compile, cargo test'
		},
		'chat.api.allowMcp': {
			type: 'boolean',
		...
		'chat.api.allowPlugins': {
			type: 'boolean',
			description: nls.localize('chat.api.allowPlugins', "Allow configured plugins in the native agent runtime."),
			default: false
		},
		...
		[ChatConfiguration.SharedLanguageServerEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.agent.sharedLanguageServer.enabled', "Controls whether the Agent session reuses the main IDE window's Singleton Language Server process (language_server_windows_x64.exe) to save RAM (~275MB)."),
			default: true,
			scope: ConfigurationScope.WINDOW,
		},
		[ChatConfiguration.SharedTsServerEnabled]: {
			type: 'boolean',
			description: nls.localize('typescript.tsserver.sharedServer.enabled', "Controls whether the TypeScript analysis server (tsserver) is shared across agent sessions and main windows to save RAM (~270MB)."),
			default: true,
			scope: ConfigurationScope.WINDOW,
		},
...
import { NativeAIAgents } from './nativeAIAgents.js';

registerWorkbenchContribution2(NativeAIAgents.name, NativeAIAgents, WorkbenchPhase.BlockRestore);
