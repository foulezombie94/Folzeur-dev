...
export class AttachContextAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor, ...args: unknown[]): Promise<void> {
		...
		const attachBtn = widget.domNode.querySelector('.monaco-action-bar .codicon-add, .monaco-action-bar .codicon-add-compact, .chat-input-toolbar')?.closest('.action-item, button, .monaco-action-bar') as HTMLElement | null;
		const inputContainer = (widget.domNode.querySelector('.chat-input-container, .interactive-input-part') as HTMLElement | null) || widget.domNode;

		// Check if popup already open
		const existingPopup = widget.domNode.querySelector('.codex-attach-popup');
		if (existingPopup) {
		...
		const items = [
			...
			},
			{
				...
				action: () => {
					...
					void commandService.executeCommand('workbench.action.openFolder');
				}
			},
			{
				icon: Codicon.target,
		...
		for (const item of items) {
			...
			dom.addDisposableListener(itemEl, dom.EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
				popup.remove();
				item.action();
		...
		setTimeout(() => {
			const doc = dom.getWindow(inputContainer).document;
			const onPointer = dom.addDisposableListener(doc, dom.EventType.POINTER_DOWN, (e) => {
				const target = e.target as Node | null;
				if (popup && target && !popup.contains(target) && !attachBtn?.contains(target)) {
					popup.remove();
					onPointer.dispose();
					onKey.dispose();
				}
