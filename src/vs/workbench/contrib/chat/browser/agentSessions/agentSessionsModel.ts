...
export class WeakRefSessionCache {
	private readonly cache = new Map<string, WeakRef<IAgentSession>>();

	public set(id: string, session: IAgentSession): void {
		this.cache.set(id, new WeakRef(session));
	}

	public get(id: string): IAgentSession | undefined {
...
export class AgentSessionsModel extends Disposable implements IAgentSessionsModel {
	...
	private async doResolveProvider(provider: string, options: { refreshProvider: boolean }, token: CancellationToken): Promise<void> {
		if (options.refreshProvider) {
			...
			// provider so that items with lazy properties are lazily resolved
			// on-demand when accessed or displayed, avoiding bulk RPC memory spikes.
