...
export class NativeToolRuntime {
	...
	public async execute(tool: INativeTool, parameters: Record<string, unknown>, cwd: string, progress?: (part: IChatProgress) => void, token: CancellationToken = CancellationToken.None, context?: NativeToolExecutionContext): Promise<unknown> {
		const policy = resolveNativeToolPolicy(tool.name, parameters);
		const key = policy.coalescible ? `${tool.name}:${hashToolParameters(parameters)}:${cwd}:${context?.conversationId ?? '<unscoped>'}` : undefined;
		if (key) {
		...
		const operation = this.runBounded(() => this.withResourceLocks(resources, async () => sanitizeResult(await tool.execute(parameters, cwd, progress, operationToken.token, context)), operationToken.token), operationToken.token);
