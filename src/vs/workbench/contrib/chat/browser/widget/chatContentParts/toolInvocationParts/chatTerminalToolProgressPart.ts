...
export class ChatTerminalToolProgressPart extends BaseChatToolInvocationSubPart implements IChatTerminalToolProgressPart {
	...
	constructor(
		...
		const isAgentTerminal = toolInvocation.toolId === 'run_in_terminal';
		if (isAgentTerminal) {
			elements.container.classList.add('chat-agent-terminal');
		}
		...
		// NativeTask terminals must always use the compact Codex-style shell card,
		// independent of the user's generic terminal preference. The agent owns a
		// hidden real process and exposes its output through this mini-terminal.
		const isSimpleTerminal = isAgentTerminal || this._configurationService.getValue<boolean>(ChatConfiguration.SimpleTerminalCollapsible);
		...
		if (isAgentTerminal) {
			this.domNode.classList.add('chat-agent-terminal');
		}
		...
		if (storedExpandedState || (!hasStoredExpandedState && this._forceExpandTerminalOutput) || (!isAgentTerminal && this._isInThinkingContainer && IChatToolInvocation.isComplete(toolInvocation) && hasStoredOutput)) {
