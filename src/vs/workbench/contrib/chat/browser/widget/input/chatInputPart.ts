...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private initSelectedModel() {
		...
		let storedSelection = getStoredSelectedModel(this.storageService, this.location, this.getSelectedModelTarget());
		// them makes the native chat wait indefinitely for a provider that no longer exists.
		if (storedSelection === 'qwen2.5-coder:7b' || storedSelection === 'copilot/qwen2.5-coder:7b') {
			this.storageService.remove(selectedModelStorageKey, StorageScope.PROFILE);
			this.logService.debug(`[ChatModelSelection] Removed retired local-agent model selection: ${storedSelection}`);
	...
	private _syncFromModel(state: IChatModelInputState | undefined, forSessionResource: URI): void {
		...
		try {
			...
			if (state?.selectedModel) {
				...
				const selectedModelId = state.selectedModel.identifier;
				if (selectedModelId === 'qwen2.5-coder:7b' || selectedModelId === 'copilot/qwen2.5-coder:7b') {
					this.logService.debug(`[ChatModelSelection] Removed retired local-agent model from conversation state: ${selectedModelId}`);
					this._inputModel?.setState({ selectedModel: undefined, modelConfiguration: undefined });
					this._modelSelectionController.clearHistoryIntent();
					this._modelSelectionController.selectDefault(sessionType);
				} else {
