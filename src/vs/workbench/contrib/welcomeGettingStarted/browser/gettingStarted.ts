...
export class GettingStartedPage extends EditorPane {
	...
	protected createEditor(parent: HTMLElement) {
		...
		this.checkAndRunOfflineAnimationOverlay();
	}

	private static hasPlayedSplashAnimation = false;

	private async checkAndRunOfflineAnimationOverlay() {
		try {
			if (GettingStartedPage.hasPlayedSplashAnimation) {
				return;
			}
			...
			if (isLoggedIn) {
				return; // User is logged into an account -> NEVER run animation
			}

			GettingStartedPage.hasPlayedSplashAnimation = true;

			const overlay = $('div.welcome-splash-overlay', {
				style: 'position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 10000; background: #000; display: flex; align-items: center; justify-content: center;'
			});

			const iframe = $('iframe', {
				src: FileAccess.asBrowserUri('vs/workbench/browser/parts/offline/splash.html').toString(true),
