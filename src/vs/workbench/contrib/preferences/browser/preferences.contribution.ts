...
import { SettingsHomeEditor } from './settingsHomeEditor.js';
import { SettingsHomeInput } from './settingsHomeInput.js';
import { ISettingsHomeSupabaseAuth } from './settingsHomeSupabaseAuth.js';
import { SettingsHomeSupabaseAuthUnavailable } from './settingsHomeSupabaseAuthUnavailable.js';

const SETTINGS_EDITOR_COMMAND_SEARCH = 'settings.action.search';

registerSingleton(ISettingsHomeSupabaseAuth, SettingsHomeSupabaseAuthUnavailable, InstantiationType.Eager);
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		SettingsHomeEditor,
		SettingsHomeEditor.ID,
		nls.localize('settingsHomeEditor', "Settings Home")
	),
	[
		new SyncDescriptor(SettingsHomeInput)
	]
);
...
class SettingsHomeInputSerializer implements IEditorSerializer {

	canSerialize(editorInput: EditorInput): boolean {
		return true;
	}

	serialize(input: SettingsHomeInput): string {
		return '';
	}

	deserialize(instantiationService: IInstantiationService): SettingsHomeInput {
		return instantiationService.createInstance(SettingsHomeInput);
	}
}
...
Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory).registerEditorSerializer(SettingsHomeInput.ID, SettingsHomeInputSerializer);
	...
	private registerSettingsActions() {
		this._register(registerAction2(class extends Action2 {
			...
			run(accessor: ServicesAccessor, args: string | IOpenSettingsActionOptions) {
				return accessor.get(IEditorService).openEditor(accessor.get(IInstantiationService).createInstance(SettingsHomeInput));
			}
		}));
		this._register(registerAction2(class extends Action2 {
			constructor() {
