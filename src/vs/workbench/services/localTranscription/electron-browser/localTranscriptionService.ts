...
export class LocalTranscriptionService {
	...
	constructor(
		...
		// Install/cache and load Tiny on the first workbench opening in the
		// background. The workbench remains usable while the worker prepares it,
		// and microphone presses reuse the resident pipeline.
		if (this.isSupported) {
			void this.prepareModel({ cacheDir: joinPath(this.environmentService.cacheHome, 'chatDictationModels').fsPath }).catch(() => { /* retry on microphone use */ });
		}
	}
	...
	prepareModel(options: Parameters<ILocalTranscriptionService['prepareModel']>[0]) { return this._getProxy().prepareModel(options); }
...
registerSingleton(ILocalTranscriptionService, LocalTranscriptionService, InstantiationType.Eager);
