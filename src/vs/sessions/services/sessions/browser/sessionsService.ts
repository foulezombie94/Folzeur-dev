...
export class SessionsService extends Disposable implements ISessionsService {
	...
	async openNewSession(options?: IOpenNewSessionOptions, token: CancellationToken = CancellationToken.None): Promise<IOpenNewSessionResult> {
		...
		// Without a folder (or when folder resolution failed above): create and activate a standalone draft
		this._startOpenSession();
		try {
			const session = this.sessionsManagementService.createNewSession(undefined, options);
			this._activate(session);
			return { session, trustDeclined: false };
		} catch (e) {
			this.logService.trace(`[SessionsView] openNewSession without folder failed:`, e);
		}
