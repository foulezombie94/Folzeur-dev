...
export class SessionsManagementService extends Disposable implements ISessionsManagementService {
	...
	private _resolveProviderForNewSession(folderUri?: URI, options?: ICreateNewSessionOptions): { provider: ISessionsProvider; sessionTypeId: string; workspace?: ISessionWorkspace } {
		...
		if (!folderUri) {
			provider = (options?.providerId ? providers.find(p => p.id === options.providerId) : undefined)
				?? providers.find(p => p.id === 'local-chat')
				?? providers.find(p => p.supportsLocalWorkspaces)
				?? providers[0];
			if (!provider) {
				throw new Error('No sessions provider available');
			}
			const sessionTypeId = options?.sessionTypeId ?? provider.sessionTypes[0]?.id ?? 'local';
			return { provider, sessionTypeId, workspace: undefined };
		}
