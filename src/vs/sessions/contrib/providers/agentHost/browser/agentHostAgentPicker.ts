	...
	constructor() {
		super({
			...
			menu: [{
				...
				group: 'navigation',
				order: -1,
				when: ContextKeyExpr.and(ContextKeyExpr.false(), IsActiveSessionAgentHost, IsPhoneLayoutContext.negate()),
			}, {
				...
				when: ContextKeyExpr.and(ContextKeyExpr.false(), ChatContextKeyExprs.isAgentHostSession, IsSessionsWindowContext, IsPhoneLayoutContext.negate(), ChatContextKeys.hasPendingDelegationTarget.negate()),
			}],
		});
	}
