...
export function getPermissionLevelMeta(level: ChatPermissionLevel): IPermissionLevelMeta {
	switch (level) {
		case ChatPermissionLevel.AutoApprove:
			return {
				label: 'Accès étendu',
				detail: 'L’agent peut effectuer la plupart des actions sans confirmation.',
				icon: Codicon.warning,
			};
		case ChatPermissionLevel.Autopilot:
			return {
				label: 'Autonome (Preview)',
				detail: 'L’agent travaille de manière autonome avec un minimum d’interruptions.',
				icon: Codicon.shield,
			};
		...
		default:
			return {
				...
				detail: 'L’agent demande votre autorisation avant les actions sensibles.',
				icon: Codicon.lock,
			};
	}
}
...
export class PermissionPicker extends Disposable {
	...
	showPicker(): void {
		...
		const levels = DEFAULT_PERMISSION_LEVELS; // Ignore delegate to enforce our 3 options
		...
		const listOptions: IActionListOptions = {
			minWidth: 400, // Make the popup wider as requested
			headerText: 'Comment les actions devront-elles être approuvées ?',
			headerLink: {
				label: 'En savoir plus',
				uri: URI.parse('https://aka.ms/vscode/docs/permissions')
			}
		};
