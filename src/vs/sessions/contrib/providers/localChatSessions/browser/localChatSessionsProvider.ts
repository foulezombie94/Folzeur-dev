	...
	trackModel(model: IChatModel, onChange: () => void): void {
		this._modelTracker.value = autorun(reader => {
			...
			if (inProgress) {
				...
				if (lastRequest?.response?.result?.errorDetails) {
...
export class LocalChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	createNewSession(workspaceUri?: URI, sessionTypeId: string = LocalSessionType.id): ISession {
	...
	createQuickChat(_sessionTypeId: string): ISession {
		return this.createNewSession(undefined, LocalSessionType.id);
	}
	...
	private async _sendFirstChat(newSession: LocalSession, chatResource: URI, options: ISendRequestOptions): Promise<ISession> {
		...
		if (result.kind === 'sent') {
			...
			result.data.responseCompletePromise.then(() => {
				const model = this.chatService.getSession(newSession.resource);
				const lastRequest = model?.getRequests().at(-1);
				const hasError = !!lastRequest?.response?.result?.errorDetails;
				newSession.setStatus(hasError ? SessionStatus.Error : SessionStatus.Completed);
				this._syncSessionFromModel(newSession);
				...
				newSession.setStatus(SessionStatus.Error);
	...
	private async _sendChildChat(primary: LocalSession, child: LocalSession, chatResource: URI, options: ISendRequestOptions): Promise<ISession> {
		...
		if (result.kind === 'sent') {
			result.data.responseCompletePromise.then(() => {
				const model = this.chatService.getSession(child.resource);
				const lastRequest = model?.getRequests().at(-1);
				const hasError = !!lastRequest?.response?.result?.errorDetails;
				child.setStatus(hasError ? SessionStatus.Error : SessionStatus.Completed);
				...
				child.setStatus(SessionStatus.Error);
	...
	private _aggregateStatus(chats: readonly IChat[], reader: IReader): SessionStatus {
		...
		for (const chat of chats) {
			if (chat.status.read(reader) === SessionStatus.Error) {
				return SessionStatus.Error;
			}
		}
		return chats[0]?.status.read(reader) ?? SessionStatus.Completed;
	}
}
