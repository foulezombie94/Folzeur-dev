...
import { NATIVE_AGENT_ID } from '../../../../../../workbench/contrib/chat/browser/nativeAIAgents.js';
...
class MockChatService extends Disposable {
	...
	readonly sendRequestCalls: { resource: URI; query: string; options: IChatSendRequestOptions | undefined }[] = [];
...
function createFixture(store: DisposableStore): ITestFixture {
	...
	config.setUserConfiguration('chat.api.provider', 'gemini');
	config.setUserConfiguration('chat.api.gemini.model', 'gemini-test');
...
suite('LocalChatSessionsProvider', () => {
	...
	test('offers only Folzeur models and disables Auto', () => {
		const store = leaks.add(new DisposableStore());
		const { instantiationService } = createFixture(store);
		...
		models.set('folzeur/gemini/gemini-test', {
			extension: new ExtensionIdentifier('folzeur.native-agent'),
			id: 'gemini-test',
			name: 'Gemini Test',
			vendor: 'folzeur',
			version: '1.0',
		...
		const snapshot = provider.getModelsSnapshot('session');
		const resolution = snapshot.desiredModelResolution;

		assert.deepStrictEqual({
			resolution: resolution.kind === 'available'
				? { kind: resolution.kind, identifier: resolution.model.identifier }
				: resolution,
			models: snapshot.models.map(model => model.identifier),
			modelTarget: snapshot.modelTarget,
			showAutoModel: provider.getModelPickerOptions('session').showAutoModel,
		}, {
			resolution: { kind: 'available', identifier: 'folzeur/gemini/gemini-test' },
			models: ['folzeur/gemini/gemini-test'],
	...
	test('createNewSession returns a session but does not show in getSessions until first send', async () => {
		...
		assert.deepStrictEqual({
			...
			modelId: newSession.modelId.get(),
			agentId: chatService.sendRequestCalls[0].options?.agentIdSilent,
		}, {
			sessionCount: 1,
			modelId: 'folzeur/gemini/gemini-test',
