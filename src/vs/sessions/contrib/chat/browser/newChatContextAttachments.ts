...
export class NewChatContextAttachments extends Disposable {
	...
	private _updateRendering(): void {
		...
		for (const entry of visibleAttachments) {
			...
			if (entry.kind === 'image' && imageData) {
				const card = dom.append(this._container, dom.$('.codex-chat-image-card'));
				card.title = entry.name;

				const img = dom.append(card, dom.$<HTMLImageElement>('img.codex-chat-image-preview'));
				...
				if (this.configurationService.getValue<boolean>(ChatConfiguration.ImageCarouselEnabled)) {
					...
					this._renderDisposables.add(registerOpenEditorListeners(card, async () => {
						await this.chatImageCarouselService.openCarouselAtResource(imageResource, imageData);
					}));
				}

				const removeButton = dom.append(card, dom.$('.codex-chat-attachment-remove'));
			...
			const card = dom.append(this._container, dom.$('.codex-chat-file-card'));
			card.title = entry.name;

			const iconBox = dom.append(card, dom.$('.codex-chat-file-icon-box'));
			if (entry.kind === 'directory') {
				iconBox.appendChild(renderIcon(Codicon.folder));
			} else {
				const ext = entry.name.split('.').pop()?.toLowerCase() || '';
				if (['html', 'xml', 'jsx', 'tsx', 'svg'].includes(ext)) {
					iconBox.appendChild(renderIcon(Codicon.code));
				} else if (['js', 'ts', 'py', 'json', 'css', 'scss', 'c', 'cpp', 'java', 'go', 'rs'].includes(ext)) {
					iconBox.appendChild(renderIcon(Codicon.fileCode));
				} else {
					iconBox.appendChild(renderIcon(Codicon.file));
				}
			}

			const info = dom.append(card, dom.$('.codex-chat-file-info'));
			const titleEl = dom.append(info, dom.$('.codex-chat-file-title'));
			titleEl.textContent = entry.name;

			const typeEl = dom.append(info, dom.$('.codex-chat-file-type'));
			if (entry.kind === 'directory') {
				typeEl.textContent = 'DOSSIER';
			} else {
				const ext = entry.name.split('.').pop()?.toUpperCase() || 'FICHIER';
	...
	async showNativeFilePicker(): Promise<void> {
	...
	showPicker(folderUri?: URI): void {
		...
		const staticPicks: (IQuickPickItem | IQuickPickSeparator)[] = [
			{
				label: localize('files', "Files and Folders..."),
	...
	private async _handleFileDialog(): Promise<void> {
		const selected = await this.fileDialogService.showOpenDialog({
			...
			canSelectMany: true,
			title: localize('selectFiles', "Sélectionner des fichiers"),
		});
		if (!selected) {
			return;
		}
