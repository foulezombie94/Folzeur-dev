 ...
 */
const DEFAULT_CHAT_PLACEHOLDER = "Que puis-je faire pour vous aujourd'hui ?";
...
export class NewChatInputWidget extends Disposable implements IHistoryNavigationWidget {
	...
	constructor(
		private readonly options: {
			...
			renderTopSlot?: (container: HTMLElement) => void;
			/**
	...
	render(parent: HTMLElement, root: HTMLElement): void {
		// Workspace trigger slot directly attached on top of the input card
		if (this.options.renderTopSlot) {
			const topWorkspaceHeader = dom.append(parent, dom.$('.new-chat-top-workspace-header'));
			this.options.renderTopSlot(topWorkspaceHeader);
			
			// Antigravity: Render the session type picker (e.g. Local button) next to the workspace picker
			const sessionTypePickerHost = dom.append(topWorkspaceHeader, dom.$('.new-chat-session-type-picker-host'));
		...
		const newChatBottomContainer = dom.append(chatInputContainer, dom.$('.new-chat-bottom-container'));
		const newChatControlsContainer = dom.append(newChatBottomContainer, dom.$('.new-chat-controls-container'));
		// Antigravity: Hide session type picker (Agent) from new chat controls entirely as requested by user
	...
	private _attachPopup: HTMLElement | undefined;
	private _attachPopupListeners = this._register(new MutableDisposable());
	...
	private _createAttachButton(container: HTMLElement): void {
		...
		const onTrigger = (e: Event) => {
			e.preventDefault();
			e.stopPropagation();
			this._toggleAttachPopup(attachButton);
		};

		this._register(dom.addDisposableListener(attachButton, dom.EventType.CLICK, onTrigger));
		this._register(dom.addDisposableListener(attachButton, dom.EventType.KEY_DOWN, (e: KeyboardEvent) => {
	...
	private _toggleAttachPopup(attachButton: HTMLElement): void {
		if (this._attachPopup) {
			this._closeAttachPopup();
			return;
		}

		const popup = dom.$('.codex-attach-popup');
		this._attachPopup = popup;

		const header = dom.append(popup, dom.$('.codex-attach-section-header'));
		header.textContent = 'Ajouter';
		...
		const items = [
			{
				...
				desc: '',
				action: () => {
					void this._contextAttachments.showNativeFilePicker();
				}
			},
			{
				icon: Codicon.folder,
				title: 'Travailler dans un projet',
				desc: 'Choisir un projet pour les nouveaux chats',
				action: () => {
			...
			},
			{
				icon: Codicon.target,
				title: 'Objectif',
				desc: 'Définir un objectif à poursuivre',
				action: () => {
					this.prefillInput('/goal ');
				}
			},
			{
				icon: Codicon.lightbulb,
				title: 'Mode plan',
				desc: 'Activer le mode plan',
				action: () => {
					this.prefillInput('/plan ');
				}
			}
		];

		for (const item of items) {
			const itemEl = dom.append(popup, dom.$('.codex-attach-item'));
			const iconEl = dom.append(itemEl, dom.$('.codex-attach-icon'));
			iconEl.appendChild(renderIcon(item.icon));

			const textEl = dom.append(itemEl, dom.$('.codex-attach-item-text'));
			const titleEl = dom.append(textEl, dom.$('.codex-attach-title'));
			titleEl.textContent = item.title;

			if (item.desc) {
				const descEl = dom.append(textEl, dom.$('.codex-attach-desc'));
				descEl.textContent = item.desc;
			}

			this._register(dom.addDisposableListener(itemEl, dom.EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
				this._closeAttachPopup();
				item.action();
		...
		const inputWrapper = attachButton.closest('.new-chat-input-area-wrapper') || attachButton.closest('.new-chat-input-container') || attachButton.parentElement || attachButton;
		...
		setTimeout(() => {
			...
			const doc = dom.getWindow(attachButton).document;
			const onPointer = dom.addDisposableListener(doc, dom.EventType.POINTER_DOWN, (e) => {
				const target = e.target as Node | null;
				if (this._attachPopup && target && !this._attachPopup.contains(target) && !attachButton.contains(target)) {
					this._closeAttachPopup();
				}
			});
			const onKey = dom.addDisposableListener(doc, dom.EventType.KEY_DOWN, (e: KeyboardEvent) => {
				if (e.key === 'Escape') {
					this._closeAttachPopup();
				}
			});
			...
			store.add(onPointer);
			store.add(onKey);
			this._attachPopupListeners.value = store;
		}, 50);
	}

	private _closeAttachPopup(): void {
		if (this._attachPopup) {
			this._attachPopup.remove();
			this._attachPopup = undefined;
		}
		this._attachPopupListeners.clear();
	}
