...
export class SessionModelSelectionModel extends Disposable implements ISessionModelSelectionModel {
	...
	selectModel(modelIdentifier: string): boolean {
		...
		try {
			...
			} else if (snapshot.modelTarget) {
				this._storageService.store(storageKey, model.identifier, StorageScope.PROFILE, StorageTarget.USER);
			}
	...
	private _refresh(trigger: ModelSelectionRefreshTrigger, session = this._session.get()): void {
		const provider = session
			? this._sessionsProvidersService.getProvider(session.providerId)
			: (this._sessionsProvidersService.getProvider('local-chat') ?? this._sessionsProvidersService.getProviders().find(p => p.supportsLocalWorkspaces) ?? this._sessionsProvidersService.getProviders()[0]);
		...
		const rememberedSelection = session
			? this._getRememberedModel(session, initialSnapshot.modelTarget)
			: (initialSnapshot.modelTarget ? this._storageService.get(getSelectedModelStorageKey(ChatAgentLocation.Chat, initialSnapshot.modelTarget), StorageScope.PROFILE) ? { identifier: this._storageService.get(getSelectedModelStorageKey(ChatAgentLocation.Chat, initialSnapshot.modelTarget), StorageScope.PROFILE)!, source: 'profile' as const } : undefined : undefined);
		...
		const desiredModelIdentifier = sessionContext.kind === 'untitled'
			...
			: (sessionModelId ?? rememberedModelId);
		...
		this._modelTarget = snapshot.modelTarget;
		const models = snapshot.models;
		const options = normalizeModelPickerOptions(provider ? provider.getModelPickerOptions(session?.sessionId ?? '') : undefined);
