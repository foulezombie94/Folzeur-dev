...
export class ModelPicker extends Disposable {
	...
	private _shouldShowPicker(): boolean {
		...
		// Keep the selector visible even before a provider has finished publishing
		// its models. This lets the user see/select AUTO and exposes the provider
		// setup state instead of silently removing the model control from the UI.
