...
export class SessionsTerminalContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// The Agent window presents the terminal in the right auxiliary rail.
		// This contribution is only registered by the Agent window, so the
		// regular IDE terminal remains in its normal bottom panel.
		const terminalView = viewDescriptorService.getViewDescriptorById(TERMINAL_VIEW_ID);
		const currentTerminalContainer = viewDescriptorService.getViewContainerByViewId(TERMINAL_VIEW_ID);
		const defaultAuxiliaryContainer = viewDescriptorService.getDefaultViewContainer(ViewContainerLocation.AuxiliaryBar);
		if (terminalView && (viewDescriptorService.getViewLocationById(TERMINAL_VIEW_ID) !== ViewContainerLocation.AuxiliaryBar || currentTerminalContainer === defaultAuxiliaryContainer)) {
			viewDescriptorService.moveViewToLocation(terminalView, ViewContainerLocation.AuxiliaryBar, 'agent-terminal-right-rail');
		}
...
class OpenSessionBrowserAction extends Action2 {

	constructor() {
		super({
			id: 'agentSession.openBrowser',
			title: localize2('openBrowser', "Open Browser"),
			icon: Codicon.globe,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const browserViews = browserViewService.getContextualBrowserViews();

		if (browserViews.size > 0) {
			// Hide the editor part instead of closing browser editors. Closing them
			// disposes their inputs and loses the visible browser tab working set.
			if (editorService.activeEditor instanceof BrowserEditorInput && layoutService.isVisible(Parts.EDITOR_PART, mainWindow)) {
				layoutService.setPartHidden(true, Parts.EDITOR_PART);
				return;
			}

			// Restore the existing browser surface and keep its current tabs/session.
			layoutService.setPartHidden(false, Parts.EDITOR_PART);
			await editorService.openEditor([...browserViews.values()][0]);
			return;
		}
...
registerAction2(OpenSessionBrowserAction);
...
class OpenSessionInTerminalAction extends Action2 {
	...
	override async run(_accessor: ServicesAccessor): Promise<void> {
		...
		const viewDescriptorService = _accessor.get(IViewDescriptorService);
		const terminalView = viewDescriptorService.getViewDescriptorById(TERMINAL_VIEW_ID);
		const currentTerminalContainer = viewDescriptorService.getViewContainerByViewId(TERMINAL_VIEW_ID);
		const defaultAuxiliaryContainer = viewDescriptorService.getDefaultViewContainer(ViewContainerLocation.AuxiliaryBar);
		if (terminalView && (viewDescriptorService.getViewLocationById(TERMINAL_VIEW_ID) !== ViewContainerLocation.AuxiliaryBar || currentTerminalContainer === defaultAuxiliaryContainer)) {
		...
		const terminalContainer = viewDescriptorService.getViewContainerByViewId(TERMINAL_VIEW_ID);

		// Toggle only the part that owns Terminal. Never reveal the default
		// Changes/Files auxiliary view as a side effect of this action.
		if (terminalContainer && viewDescriptorService.getViewLocationById(TERMINAL_VIEW_ID) === ViewContainerLocation.AuxiliaryBar && viewsService.isViewVisible(TERMINAL_VIEW_ID)) {
		...
		// openView selects Terminal's own generated auxiliary container, so the
		// Files/Changes container is never opened by the terminal button.
		if (terminalContainer) {
			await viewsService.openViewContainer(terminalContainer.id, true);
		}
