...
import { IInstantiationService, refineServiceDecorator, ServicesAccessor, ServiceIdentifier } from '../../platform/instantiation/common/instantiation.js';
...
export class Workbench extends Disposable implements IAgentWorkbenchLayoutService {
	...
	startup(): IInstantiationService {
		try {
			...
			instantiationService.invokeFunction(accessor => {
				const safeGet = <T>(id: ServiceIdentifier<T>): T | undefined => {
					try { return accessor.get(id); } catch { return undefined; }
				};

				const lifecycleService = safeGet<ILifecycleService>(ILifecycleService);
				const hostService = safeGet<IHostService>(IHostService);
				const hoverService = safeGet<IHoverService>(IHoverService);
				const dialogService = safeGet<IDialogService>(IDialogService);
				const markdownRendererService = safeGet<IMarkdownRendererService>(IMarkdownRendererService);
				...
				// Register Listeners
				if (lifecycleService && hostService && dialogService) {
	...
	private initServices(serviceCollection: ServiceCollection): IInstantiationService {
		...
		// Lifecycle Service
		serviceCollection.set(ILifecycleService, new SyncDescriptor(BrowserLifecycleService, []));

		// Host Service
		serviceCollection.set(IHostService, new SyncDescriptor(BrowserHostService, []));
		...
		instantiationService.invokeFunction(accessor => {
			try {
				...
				// Ignore if ILifecycleService is not registered in this custom window
			}
		});
	...
	initLayout(accessor: ServicesAccessor): void {
		...
		(this.paneCompositeService as { initialize?(): void }).initialize?.();
