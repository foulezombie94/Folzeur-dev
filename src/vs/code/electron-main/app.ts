...
import { ISupabaseAuthMainService, SUPABASE_AUTH_CHANNEL_NAME } from '../../platform/supabase/common/supabaseAuth.js';
import { SupabaseAuthChannel } from '../../platform/supabase/common/supabaseAuthIpc.js';
import { SupabaseAuthMainService } from '../../platform/supabase/electron-main/supabaseAuthMainService.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		services.set(ISupabaseAuthMainService, new SyncDescriptor(SupabaseAuthMainService, undefined, false /* proxied to renderer processes */));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Supabase authentication
		mainProcessElectronServer.registerChannel(SUPABASE_AUTH_CHANNEL_NAME, new SupabaseAuthChannel(accessor.get(ISupabaseAuthMainService)));
	...
	private async openFirstWindow(accessor: ServicesAccessor, initialProtocolUrls: IInitialProtocolUrls | undefined): Promise<ICodeWindow[]> {
		...
		// Always start in the main IDE. The Agent window remains available through
		// its explicit in-IDE action, but never replaces the IDE at startup.
