...
import { LocalAppServerChannel, LOCAL_APP_SERVER_CHANNEL } from '../../../platform/localApp/common/localAppServer.js';
import { LocalAppServerService } from '../../../platform/localApp/node/localAppServerService.js';
...
class SharedProcessMain extends Disposable implements IClientConnectionFilter {
	...
	private initChannels(accessor: ServicesAccessor): void {
		...
		// Local static application preview. The HTTP listener intentionally lives in
		// this Node shared process; the renderer must not depend on Node built-ins.
		const localAppServer = this._register(accessor.get(IInstantiationService).createInstance(LocalAppServerService));
		this.server.registerChannel(LOCAL_APP_SERVER_CHANNEL, new LocalAppServerChannel(localAppServer));
