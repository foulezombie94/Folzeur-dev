...
onceDocumentLoaded(() => {
	...
	function navigateTo(rawUrl: string): void {
		...
		try {
			if (!finalUrl.includes(' ') && !finalUrl.startsWith('http://') && !finalUrl.startsWith('https://') && finalUrl.includes('.')) {
				finalUrl = 'https://' + finalUrl;
			}
			new URL(finalUrl);
		} catch {
			// If not a valid URL, fallback to Google search (igu=1 allows iframe embedding)
			finalUrl = `https://www.google.com/search?q=${encodeURIComponent(rawUrl)}&igu=1`;
		}
		...
		vscode.setState({ url: finalUrl });
		if (input.value !== finalUrl) {
